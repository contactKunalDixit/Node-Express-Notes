...
Todo: But the beauty of Express is that it can parse it and auto creates 2 objects (and thier associated properties and methods) which are automatically be passed as arguements in app.use()
...
*/
/*
* NOTE: imp tip: whenever you make any change in the js file...make sure to re run the file in node terminal */
/*
 * Also, we can either have app.use((req,res)=>{}) or app.get('path', callbackFunc)
*
There can only be one res.send() because once you have sent something back to the clien, that process is taken careOff. If you have multiple res s, then it leads to error so only one res.send()
...
// **********************************************************
/*
*app.get() exoects two parameters , 1st the path that we are matching e.g. "/cats" and then the callback function  - absolutely the same as used with app.use(req,res) with the logic that this callback function will run when the incoming requests matches the stated mentioned path(1st parameter)
*/
...
// **********************************************************
// * POST REQUEST  - USE POSTMAN to see the post request and response in execution
// **********************************************************
...
* This property is (GIVES) an object containing properties mapped to the named route “parameters”. For example, if you have the route /user/:name, then the “name” property is available as req.params.name. This object defaults to {}.

It tells you (in an Object form), what route path has been mentioned and captures it therof in an Object form

 */

app.get('/c/:paramVar', (req, res) => {
  console.dir(req.params);
  console.log(req.params);
  console.log('This is req.param example');
  res.send('This is a req.param example');
});
